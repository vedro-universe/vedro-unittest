...
__all__ = ("UnitTestScenarioProvider",)
...
class UnitTestScenarioProvider(ScenarioProvider):
    def __init__(self, raise_as_exc_group: bool = IS_PY311_PLUS) -> None:
        """
    ...
    async def provide(self, source: ScenarioSource) -> List[VirtualScenario]:
        ...
        scenarios = self._collect_scenarios(module, module_path=source.path)
        return [create_vscenario(scn, project_dir=source.project_dir) for scn in scenarios]

    def _collect_scenarios(self, module: ModuleType, module_path: Path) -> List[Type[Scenario]]:
    ...
    def _build_vedro_scenario(self, test: Union[TestSuite, TestCase], *,
                              path: Path, name: str, subject: str) -> Type[Scenario]:
        """
